import {bigBytesInt, bigStringInt, bufferConcat, bytesFromBigInt} from './bin.js';
import {getDeferred} from './mtproto';

const RSA_PUBLIC_KEYS = {
  '14101943622620965665': {
...
-----END PUBLIC KEY-----`,
    ...
    exponent: '010001',
    fingerprint: '12587166101702965583'
  },
...
function selectRsaPublicKey(fingerprints) {
  for (const fingerprint of fingerprints) {
    if (RSA_PUBLIC_KEYS[fingerprint]) {
      return RSA_PUBLIC_KEYS[fingerprint];
    }
  }
  return false;
}
...
function rsaEncrypt (publicKey, bytes) {
  ...
  const N = bigStringInt(publicKey.modulus, 16);
  const E = bigStringInt(publicKey.exponent, 16);
  const X = bigBytesInt(bytes);
  const encryptedBigInt = X.modPowInt(E, N);
  return bytesFromBigInt(encryptedBigInt, 256);
}

function sha1Hash(bytes) {
  return crypto.subtle.digest('SHA-1', bytes);
}
...
function aesIgeEncrypt(bytes, keyBytes, ivBytes) {
  return workerTask('aes_ige_encrypt', [bytes, keyBytes, ivBytes]);
}

function aesIgeDecrypt(bytes, keyBytes, ivBytes) {
  return workerTask('aes_ige_decrypt', [bytes, keyBytes, ivBytes]);
}
