...
export default class MTProto {
  ...
  async encryptMessages(messages) {
    ...
    const ecryptedData = aesIgeEncrypt(dataWithPadding, aesKey, aesIv);
  ...
  async decryptMessage(encryptedMessage, msgKey) {
    ...
    const dataWithPadding = bytesToArrayBuffer(aesIgeDecrypt(encryptedMessage, aesKey, aesIv));
  ...
  async sendPlainRequest(buffer) {
    ...
    this.plainRequestDeffered = getDeferred();
    this.transportSend(reqBuffer);
    const response = await this.plainRequestDeffered.promise;
    delete this.plainRequestDeffered;

    return this.unwrapPlainResponse(response, 'Object');
  }

  handlePlainResponseMessage(buffer) {
    if (this.plainRequestDeffered) {
      this.plainRequestDeffered.resolve(buffer);
    }
  }

  handleResponseMessage(object, msgId) {
    const sentMessage = this.sentMessages.get(msgId);
    if (sentMessage) {
    ...
    switch (object._) {
      case 'msg_container':
        for (const msg of object.messages) {
          this.handleResponseMessage(msg.body, msg.msg_id, msg.seq_no);
        }
        break;
    ...
    if (sentMessage) {
      sentMessage.deferred.resolve(object);
    }
  }
